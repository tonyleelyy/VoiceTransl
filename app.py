...
class MainWindow(QMainWindow):
    ...
    def cancel_task(self):
        self.status.emit(_("status_cancelling"))
        try:
            if self.worker:
                self.worker.stop()
        except Exception as e:
            self.status.emit(_("status_cancel_worker_error", error=e))
        ...
        try:
            ...
            self.status.emit(_("status_cancel_thread_error", error=e))

        self.status.emit(_("status_cancel_done"))
        ...
        try:
            ...
            if not os.path.exists(LOG_PATH):
                if not self.file_not_found_message_shown:
                    ...
                    self.log_display.setPlainText(f"[{timestamp}] " + _("log_file_not_found", path=LOG_PATH) + "\n")
            ...
            if not self.file_not_found_message_shown:
                timestamp = QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss")
                self.log_display.setPlainText(f"[{timestamp}] " + _("log_not_found_retry", path=LOG_PATH) + "\n")
                self.file_not_found_message_shown = True
            self.last_read_position = 0
        except IOError as e:
            timestamp = QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss")
            self.log_display.appendPlainText(f"[{timestamp}] " + _("log_io_error", error=e) + "\n")
            # 可以考虑在IO错误时停止timer或做其他处理
        except Exception as e:
            timestamp = QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss")
            self.log_display.appendPlainText(f"[{timestamp}] " + _("log_unknown_error", error=e) + "\n")
    ...
    def cleaner(self):
        self.status.emit(_("status_cleaning_intermediate"))
        ...
        self.status.emit(_("status_cleaning_output"))
