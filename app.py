...
class ConcurrentTranslationPool:
    ...
    def __init__(self, project_dir, base_config_path, max_concurrent, stop_event,
        ...
        # 本地模型相关
        self._local_model_ports = {}  # thread_id -> port
        self._local_model_procs = {}  # thread_id -> process
        self._local_model_lock = threading.Lock()
        # 串行模式相关 (max_concurrent <= 1)
        self._serial_mode = max_concurrent <= 1
        ...
        if self._serial_mode:
            ...
            with self._serial_lock:
                ...
                if self._local_model_config and self._local_model_config.get('sakura_file'):
                    proc, local_port = self._start_local_model(0)
                    if proc:
                        self._local_model_procs[0] = proc
                
                try:
                    # 执行翻译
                    self._translate_one(tf, 0, local_port)
                finally:
                    # 停止本地模型
                    if proc:
                        self._stop_local_model(0)
        else:
            # 并发模式：放入队列
            self._queue.put(tf)
    ...
    def stop(self):
        ...
        # 停止串行模式的本地模型
        if self._serial_mode:
            if self._serial_proc:
                try:
                    if self._serial_proc.poll() is None:
    ...
    def _start_local_model(self, thread_idx):
        ...
        # 为每个线程分配独立端口 (8989 + thread_idx)
        port = 8989 + thread_idx
        ...
        self._status_callback(f"[INFO] 线程 {thread_idx} 正在启动本地模型，端口 {port}...")
        ...
        try:
            ...
            while not self._stop_event.is_set():
                    ...
                    if chat_resp.status_code == 200:
                        try:
                            ...
                            if isinstance(body, dict) and body.get("choices"):
                                model_ready = True
                                self._status_callback(f"[INFO] 线程 {thread_idx} 本地模型已就绪，端口 {port}")
                                break
                        except Exception:
                            pass
                ...
                if time() - start_wait > 120:
                    self._status_callback(f"[ERROR] 线程 {thread_idx} 本地模型启动超时")
                    try:
                        proc.terminate()
                        proc.wait(timeout=3)
                    except Exception:
            ...
            self._status_callback(f"[ERROR] 线程 {thread_idx} 启动本地模型失败: {e}")
            return None, None

    def _stop_local_model(self, thread_idx):
        """停止指定线程的本地模型"""
        with self._local_model_lock:
            proc = self._local_model_procs.pop(thread_idx, None)
        if proc:
            try:
    ...
    def _stop_local_models(self):
        """停止所有本地模型"""
        for thread_idx in list(self._local_model_procs.keys()):
            self._stop_local_model(thread_idx)
    ...
    def _worker_loop(self, thread_idx):
        ...
        local_port = None
        if self._local_model_config and self._local_model_config.get('sakura_file'):
            proc, local_port = self._start_local_model(thread_idx)
            if proc:
                with self._local_model_lock:
                    self._local_model_procs[thread_idx] = proc
        ...
        try:
            while not self._stop_event.is_set():
                ...
                try:
                    self._translate_one(tf, thread_idx, local_port)
                finally:
                    self._queue.task_done()
        finally:
            # 停止本地模型
    ...
    def _translate_one(self, tf, thread_idx, local_port):
        ...
        # 准备独立配置文件
        self._prepare_config(workspace, local_port)
    ...
    def _prepare_config(self, workspace, local_port=None):
        ...
        # 如果是本地模型多线程模式，替换端口号
        if local_port:
            # 替换默认端口号 8989 为当前线程的端口
            content = content.replace('http://localhost:8989', f'http://localhost:{local_port}')
...
""")
    ...
    def initAdvancedSettingTab(self):
        ...
        model_row.addWidget(BodyLabel("🤖 用于翻译的模型类别："))
        ...
        model_row.addWidget(BodyLabel("🤖 翻译最大并发数（0为串行，1以上为并发）："))
...
class MainWorker(QObject):
    ...
    def run(self):
        ...
        engine = 'gpt35-1106'
        if need_translate:
            engine = 'sakura-010' if 'sakura' in translator else 'gpt35-1106'
